...
namespace Assets.SuperGoalie.Scripts.States.GoalKeeperStates.InterceptShot.MainState
{
    public class InterceptShotMainState : BState
    {
        ...
        public override void Enter()
        {
            ...
            // Detect corner shots (when ball is far to the sides)
            bool isCornerShot = Mathf.Abs(_relativeBallPositionAtPlayerInterceptPoint.x) > 2.0f;

            // Check if this is a strong keeper and set multipliers accordingly
            bool isStrongKeeper = Owner.IsStrongKeeper;
            
            // Set multipliers based on keeper strength
            float baseDiveMultiplier = isStrongKeeper ? 1.5f : 1.0f;
            float cornerDiveMultiplier = isStrongKeeper ? 2f : 1.5f;
            float diveMultiplier = isCornerShot ? cornerDiveMultiplier : baseDiveMultiplier;
            float baseDiveSpeed = isStrongKeeper ? 45f : 30f;
            float speedMultiplier = isCornerShot ? (isStrongKeeper ? 0.8f : 1.3f) : 1.2f;
            float baseHeightMultiplier = isStrongKeeper ? 1.5f : 1.1f;
            float cornerHeightMultiplier = isStrongKeeper ? 0.3f : 1.3f;
            float heightMultiplier = isCornerShot ? cornerHeightMultiplier : baseHeightMultiplier;
            float reachMultiplier = isCornerShot ? (isStrongKeeper ? 1.8f : 1.3f) : 1.1f;
            float jumpDistance = Owner.JumpDistance * diveMultiplier;

            //find the distance that the player can jump and still reach the ball
            if (playerDistanceToOrthogonalPoint >= Owner.Reach)
                maxMovementDistanceOfPlayer = Mathf.Clamp((playerDistanceToOrthogonalPoint - Owner.Reach) * diveMultiplier, 0f, jumpDistance);
            else
                maxMovementDistanceOfPlayer = playerDistanceToOrthogonalPoint * diveMultiplier;
            ...
            float playerRawDiveSpeed = (maxMovementDistanceOfPlayer / _timeOfBallToPlayerOrthogonalPointOnBallPath) * speedMultiplier;
            float playerDiveSpeed = Mathf.Clamp(playerRawDiveSpeed, baseDiveSpeed * 0.7f, baseDiveSpeed * (isStrongKeeper ? 5.5f : 5.5f));

            //calculate the jump height required to reach the ball
            float playerRawJumpHeight = (_relativeBallPositionAtPlayerInterceptPoint.y - Owner.Height) * heightMultiplier;
            float maxJumpHeight = Owner.JumpHeight * (isStrongKeeper ? 5.8f : 5.3f);
            float playerJumpHeight = Mathf.Clamp(playerRawJumpHeight, 0f, maxJumpHeight);
            ...
            // حذف تغییر سرعت - از سرعت پیش‌فرض استفاده کن
             Owner.RPGMovement.Speed = playerDiveSpeed;
            Owner.RPGMovement.SetSteeringOn();

            //set the height animator value with extended reach for corners
            float maxHeight = isStrongKeeper ? 1.5f : 1.1f;
            _height = Mathf.Clamp((_relativeBallPositionAtPlayerInterceptPoint.y / Owner.JumpReach) * reachMultiplier, 0f, maxHeight);
        ...
        public override void Exit()
        {
            ...
            // بازگردانی سرعت به مقدار پیش‌فرض
            Owner.RPGMovement.Speed = 6;  // سرعت پیش‌فرض
