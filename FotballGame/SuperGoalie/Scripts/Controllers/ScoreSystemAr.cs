...
{
    public class ScoreSystemAr : MonoBehaviour
    {
        // UI Panels
        ...
        private RtlText[] singlePlayerScoreTexts;

        [Header("Multi Player")] [SerializeField]
        private RtlText[] multiPlayerScoreTexts;
        ...
        [Header("Sound Effects")] public AudioClip turnChangeSound; // Assign this in the Unity Inspector
        ...
        private void Start()
        {
            ...
            if (redTeamScoreText != null)
                redTeamScoreText.text = $"گل‌ها: {ToPersianNumber(redGoals)}";

            if (blueTeamScoreText != null)
                blueTeamScoreText.text = $"گل‌ها: {ToPersianNumber(blueGoals)}";
        }
        ...
        public void StartMode(bool twoPlayers)
        {
            ...
            else
            {
                ...
                ShowTurnIndicator(isRedTurn ? 1 : 2);
        ...
        private void EndGame()
        {
            ...
            if (!twoPlayerMode && gameOverPanelSingle != null)
            {
                ...
                if (singlePlayerScoreTexts != null && singlePlayerScoreTexts.Length > 0 &&
                    singlePlayerScoreTexts[0] != null)
                {
                    // Show 'گل‌ها: ' before the score number
                    singlePlayerScoreTexts[0].text = $"گل‌ها: {ToPersianNumber(currentScore)}";
            ...
            else if (twoPlayerMode && gameOverPanelMulti != null)
            {
                ...
                // Team images are now shown without happy/sad animations
                ...
                if (multiPlayerScoreTexts != null && multiPlayerScoreTexts.Length >= 2)
                {
                    ...
                    if (multiPlayerScoreTexts[0] != null)
                        multiPlayerScoreTexts[0].text = $"گل‌ها: {ToPersianNumber(redGoals)}";

                    // Second text for blue score
                    if (multiPlayerScoreTexts[1] != null)
                        multiPlayerScoreTexts[1].text = $"گل‌ها: {ToPersianNumber(blueGoals)}";
        ...
        {
            if (scoreText != null) scoreText.text = "گل: " + ToPersianNumber(currentScore);
        }
        ...
        public void UpdateTurnTextColors()
        {
            if (!twoPlayerMode)
            {
                ...
                if (_GameManager != null && _GameManager._scoreText != null)
                    _GameManager._scoreText.color = neutralTextColor;
                return;
            }
